...
async function startServer() {
    ...
    try {
        ...
        db = client.db("admin_db"); // El nombre de la base de datos es admin_db
...
1.  **Crea el archivo `server.js`** en la raíz de tu proyecto y pega el código de arriba.
2.  **Elimina la carpeta `api`** que contiene los archivos `add-user.js` y `login.js`.
3.  **Sube los cambios a Git:**

```bash
git add .
git commit -m "Solución definitiva: Proyecto configurado para usar un solo servidor."
git push
